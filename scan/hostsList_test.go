	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if hl.Hosts[1] != tc.host {
				t.Errorf("Expected host name %q ad index 1, got %q instead\n", tc.host, hl.Hosts[1])
			}
		})
	}
}
...
func TestSaveLoad(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("An error while creating a temp file")
	}
	defer func(name string) {
		err := os.Remove(name)
		if err != nil {
			t.Fatalf("An error while removing the temp file %s: %q", name, err)
		}
	}(tf.Name())

	if err := hl1.Save(tf.Name()); err != nil {
		t.Fatalf("An error while saving the list to the file: %q", err)
	}

	if err := hl2.Load(tf.Name()); err != nil {
		t.Fatalf("An error while getting a list from the file: %q", err)
	}

	if hl1.Hosts[0] != hl2.Hosts[0] {
		t.Errorf("Host %q should match expected host %q", hl2.Hosts[0], hl1.Hosts[0])
	}
}
...
func TestLoadNoFile(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("An error while creating a temp file: %q", err)
	}

	if err := os.Remove(tf.Name()); err != nil {
		t.Fatalf("An error while deleting the temp file %q - %q", tf.Name(), err)
	}
